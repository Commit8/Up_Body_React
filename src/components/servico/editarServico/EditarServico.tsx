...
function EditarServico() {
  ...
  function atualizarEstado(e: ChangeEvent<HTMLInputElement>) {
    setServico({
      ...servico,
      [e.target.name]: e.target.name === "valor" ? Number(e.target.value) : e.target.value,
    });
  }
  ...
  async function enviar(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (error.toString().includes("401")) {
        ...
        ToastAlerta("Erro ao atualizar serviço.", "erro");
      }
    }
  ...
  return (
    <div className="flex justify-center items-center min-h-screen bg-[#121212] text-white p-6">
      ...
      >
        <h2 className="text-2xl font-bold text-center">Editar Serviço</h2>
        ...
        >
          {isLoading ? <ClipLoader color="#fff" size={24} /> : "Salvar Alterações"}
        </button>
      </form>
    </div>
  );
